...
| HEALTH-003 | Alto | aberto | O núcleo cíclico de 133 arquivos inclui `app.ts`, `routes/index.ts`, `libs/socket.ts`, `libs/wbot.ts`, `wbotMessageListener.ts` e a maioria dos controllers. É a causa estrutural de o projeto não escalar horizontalmente e de o teste unitário exigir subir meio sistema. Não se resolve por decreto nem num lote: quebrar o ciclo exige inverter dependências (depender de interface, não de implementação) começando pelos pontos de maior fan-in. |
