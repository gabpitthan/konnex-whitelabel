...
| HEALTH-001 | Médio | parcial | **Grafo de dependências medido em 2026-08-08** com `graphify`: 636 arquivos do backend, e **133 deles (21%) formam um único núcleo mutuamente cíclico** — cada um alcança todos os outros e volta, então nenhum pode ser entendido, testado ou substituído sozinho. Os outros 470 arquivos estão limpos. Maior fan-in: `errors/AppError.ts` (208), `models/User.ts` (94), `models/Whatsapp.ts` (82). `scripts/code-health.sh` passa a reportar isso em vez de NAO MEDIDO. **Continua aberto:** o gate de **carga** (RPS, p95/p99) segue sem ferramenta e sem nenhuma medição. |
| HEALTH-003 | Alto | mitigado | **Aberto na 1.39: núcleo de 133 → 26 arquivos**, arquivos sem ciclo de 470 → 589, zero arestas reais subindo de camada. Seis correções: inversão dos ganchos de sessão do WhatsApp, extração de funções puras, e três módulos movidos para a camada correta. **Resta:** um componente de 26 e outro de 17 arquivos, com ciclos *dentro* da mesma camada (serviço↔serviço) — natureza diferente e gravidade menor. A reconexão por queda real de conexão não foi exercitada. |
...
| PLAN-001 | Médio | parcial | `requirePlanFeature` impõe `useCampaigns`, `useSchedules`, `useInternalChat` e `useIntegrations` em 44 rotas (1.35). **Faltam** `useKanban`, `useOpenAi`, `useExternalApi`, `useWhatsapp`, `useFacebook` e `useInstagram` — o middleware existe e aplicá-las é mecânico, ficou fora do lote para não crescer sem verificação. O guarda faz uma consulta por requisição protegida; cachear por empresa se virar gargalo. |
