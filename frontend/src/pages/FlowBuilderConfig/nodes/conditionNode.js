...
export default memo(({ data, isConnectable }) => {
  ...
  return (
    <div style={{backgroundColor: 'var(--surface-raised)', padding: '8px', borderRadius: '8px'}}>
      ...
      <div style={{color: 'var(--surface-sunken)', fontSize: '12px'}}>{data.key}</div>
      <div style={{color: 'var(--surface-sunken)', fontSize: '12px'}}>{typeCondition(data.condition)}</div>
      <div style={{color: 'var(--surface-sunken)', fontSize: '12px'}}>{data.value}</div>
      <Handle
        type="source"
        position="right"
        id="a"
        style={{ top: 10, background: "var(--brand-base)" }}
        isConnectable={isConnectable}
      />
      <Handle
        type="source"
        position="right"
        id="b"
        style={{ bottom: 10, top: "auto", background: "var(--brand-base)" }}
        isConnectable={isConnectable}
      />
    </div>
  );
